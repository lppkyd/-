...
Page({
  ...
  lookupWord() { 
    ...
    if (!word.trim()) { wx.showToast({ title: '请先输入单词', icon: 'none' }); return; } 
    this.setData({ isLooking: true }); 
    dictionaryApi.queryWord(word).then((wordObj) => { 
      this.setData({ trans: wordObj.trans.join('\n'), autoFilled: true, isLooking: false }); 
      wx.showToast({ title: '查询成功', icon: 'success', duration: 1500 });
    }).catch((err) => { 
      console.error('查词失败:', err); 
      this.setData({ isLooking: false }); 
      ui.showError(err.message || '未找到释义，请手动输入', '查询失败');
    }) 
  },

  async saveWord() {
    const { word, trans, loginUser } = this.data; 
    ...
    // 完美对齐 SavedWordController
    const payload = { userId: loginUser.userId, word: wordObj.name, trans: trans.trim(), usphone: '', ukphone: '', example: '' };
